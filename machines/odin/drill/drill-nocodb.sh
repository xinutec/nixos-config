...
#                                     #   ssh -L 8449:127.0.0.1:8449 odin.xinutec.org
...
# something unrelated to the actual defect.
...
docker run -d --name "$CONTAINER" \
  ...
  -v "$DEST:/usr/app/data" \
  "$prod_image" >/dev/null || fail "docker run failed"

log "waiting for nocodb on 127.0.0.1:$PORT ..."
ready=0
for i in $(seq 1 60); do
  code=$(curl -sS -o /dev/null -w "%{http_code}" "http://127.0.0.1:$PORT/dashboard" 2>/dev/null || echo 000)
  case "$code" in
...
live_bases=$(q "$DEST/noco.db" "SELECT count(*) FROM nc_bases_v2")
live_objects=$(q "$DEST/noco.db" "SELECT count(*) FROM sqlite_master")
...
ic=$(sqlite3 "$DEST/noco.db" "PRAGMA integrity_check")
[ "$ic" = "ok" ] || fail "restored DB failed integrity_check after startup: $ic"

log "ALL CHECKS PASSED — $live_bases bases restored and served by $prod_image"
if [ "$KEEP" -eq 1 ]; then
  log "left running on 127.0.0.1:$PORT (--keep); remove with: docker rm -f $CONTAINER"
fi
